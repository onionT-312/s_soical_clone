...
class ChatDataSource {
  ...
  Future<ChatSessionModel> createChatSession(
      ChatSessionModel chatSession) async {
    try {
      final saveChatSession = chatSession.copyWith(
        chatId: chatSession.chatId,
      );
      DocumentReference<dynamic> chatDoc =
          _chatCollection.doc(saveChatSession.chatId);
  ...
  Future<void> createUserChatMap(
      String userId, Map<String, MessageModel?> userChatMap) async {
    try {
      final userDoc = firestoreDatabase
          .collection(FirestoreCollectionConstants.userList)
          .doc(userId);
      final userChatMapJson =
          userChatMap.map((key, value) => MapEntry(key, value?.toJson()));
  ...
  Future<Map<String, MessageModel?>?> getUserChatMap(String userId) async {
    try {
      ...
      final userChatMapJson =
          userChatMap.data()!['userChatMap'] as Map<String, dynamic>;
      final userChatMapModel = userChatMapJson.map((key, value) =>
          MapEntry(key, value != null ? MessageModel.fromJson(value) : null));
  ...
  Future<Map<String, MessageModel?>?> updateUserChatMap(
      String userId, String recipientId, MessageModel message) async {
    try {
      final userDoc = firestoreDatabase
          .collection(FirestoreCollectionConstants.userList)
          .doc(userId);
      ...
      final userChatMapModel = userChatMapJson.map((key, value) =>
          MapEntry(key, value != null ? MessageModel.fromJson(value) : null));
      userChatMapModel.update(recipientId, (value) => message,
          ifAbsent: () => message);
      final userChatMapJsonUpdated =
          userChatMapModel.map((key, value) => MapEntry(key, value?.toJson()));
      await userDoc.set({
